...
c     Temporary insert of code segment to extract coordinates of "deformed" surfaces
         if(nhhp(i).gt.0)then 
             write (dfilename, fmt='(a,i1,a)') 'def_surf_', i, '.dat'
             dfid = 1000  
             open(dfid,file=dfilename)      
			 do j=nwp0,nwp
				 write(dfid,*) rw(1,j)+drw(2,j),rw(2,j)+drw(2,j)
			 enddo 
			 close(dfid) 
         endif  
         nwp0=nwp+1
